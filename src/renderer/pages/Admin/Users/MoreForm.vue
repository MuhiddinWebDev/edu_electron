...
const adder = ()=>{
  form_data.value.users.push({
    ...
    confirmPassword: "",
    lang: langAuto.value,
    filial_id: findRole.value != "SuperAdmin" ? branchID.value : branchAuto.value, 
    status:'success'
  });
}
const oneAdd = () => {
              ...
              <tr v-for="(item, index) in form_data.users">
                ...
                <td>
                  <n-select
                    ...
                    label-field="name"
                    value-field="role"
                    :status="checkValidation(item.role, 'Huquqi', 3)"
                    style="min-width: 150px"
                ...
                <td>
                  <n-input
                    ...
                    :minlength="3"
                    :status="checkValidation(item.confirmPassword, 'Qayta parol', 5)"
                    show-count
                    clearable
                  />
                </td>
                <td>
                  <n-switch @update:value="updateLang($event,item.id)"  :rail-style="railStyle">
                    <template #checked> Russian </template>
                    <template #unchecked> Uzbek </template>
                  </n-switch>
                </td>
