...
onMounted(async () => {
  chefs.value = await Chefservice.getAll();
});

const getReport = () => {
  axios
    .post(`/akt-sverka/chef-act`, {
...
const openTopDrawer = (data) => {
  ...
  } else if(data.doc_type == 'pay_chef') {
  ...
  <div class="table">
    <table v-if="reports" id="customers">
      ...
      <tr v-for="(dt, index) in reports.items" @click="openTopDrawer(dt)">
        <td>
          #{{ dt.doc_id }} {{ dt.doc_type == 'order' ? 'Savdo xujjati': dt.doc_type == 'pay_chef' ? "Oshpazga to'lov": dt.doc_type == 'emp_salary' ? "Xodimlarga to'lov belgilash": dt.doc_type }}
          {{ moment(dt.datetime * 1000).format("YYYY-MM-DD hh:mm:ss") }}
        </td>
        <td>{{ dt.chef.full_name }}</td>
        <td>
          {{
            new Intl.NumberFormat("ru-RU", { style: "decimal" }).format(
              dt.tolandi
