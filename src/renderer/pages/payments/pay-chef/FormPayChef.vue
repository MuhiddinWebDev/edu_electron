...
<script setup>
import PayChefService from "../../../services/pay-chef.service";
...
onMounted(() => {
  if (props.type == "update" || props.type == "sverka") {
    PayChefService.getOne(props.id).then((res) => {
      form_data.value.datetime = res.datetime;
      form_data.value.chef_id = res.chef_id;
...
const changeChief = (e) => {
  const chief = chefs.value.find((val) => val.id == e);
  form_data.value.chef_id = e;
...
const save = () => {
  if (props.type == "create") {
    PayChefService.create(form_data.value).then((res) => {
      emits("create");
    });
  } else {
    PayChefService.update(props.id, form_data.value)
      .then((res) => {
        emits("update");
      })
      .catch((err) => {
...
const chooseChief = (row) => {
  form_data.value.chef_id = row.id;
  showAddChief.value = false;
};

const createChief = (data) => {
  chefs.value.push(data);
  form_data.value.chef_id = data.id;
  showAddChief.value = false;
};
</script>
